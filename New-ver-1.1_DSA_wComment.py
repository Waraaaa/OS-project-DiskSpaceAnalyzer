...
# =======================
# Calculates the total size of all files in the selected folder (recursive).
# This will ignores symbolic links (shortcuts or junctions) and inaccessible files.
# =======================
...
def analyze(base_path="/"):
    ...
    scanned = set()  # To avoid scanning duplicate paths (like symlinks).
    disk_data = []
    ...
    for item in os.listdir(base_path):
        ...
        try:
            ...
            # Optionally skip certain file types (e.g., temp files)
            SKIP_EXTENSIONS = [".tmp"]
...
def input_case(drives):
    ...
    print("Please select the drive. Type out the number: ")
    while True:
        number = input()
        if number == "exit": exit()
...
# Will detects available drives and have the user to select one.
...
# This is the part that gets executed when the script runs.
# =======================
if __name__ == "__main__":
    main()
    # analyze("D:\Riot Games")  # You can use this line to directly analyze a path (for testing)
